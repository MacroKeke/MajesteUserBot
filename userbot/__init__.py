...
if HEROKU_APPNAME is not None and HEROKU_APIKEY is not None:
    if EZZEC == True:
        ...
        heroku_var["UPSTREAM_REPO_URL"] = "https://github.com/MacroKeke/MajesteUserBot.git"
else:
    app = None
...
UPSTREAM_REPO_URL = "https://github.com/MacroKeke/MajesteUserBot.git" 
EMERGENCY = "https://github.com/MacroKeke/MajesteUserBot.git" # Acil durrum için
...
PAKET_ISMI = os.environ.get("PAKET_ISMI", "| 🌃 @majesteuserbot Paketi |")
...
WHITELIST = get('https://raw.githubusercontent.com/MacroKeke/datas/main/whitelist.json').json()
...
URL = 'https://rawcdn.githack.com/MacroKeke/datas/54e3fc8b1087f6e52344e8eadba798d93097c3c3/force-surum.check' 
...
# Buraya da ellemeyiniz ! #
    ...
    try:
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@majesteuserbot":
                rev_text = query[::-1]
                veriler = (butonlastir(0, sorted(CMD_HELP)))
                result = await builder.article(
                    f"Lütfen Sadece .yardım Komutu İle Kullanın",
                    text=f"**En Gelişmiş UserBot!** [Majeste](https://t.me/majesteuserbot) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** 1/{veriler[0]}",
                    buttons=veriler[1],
                    link_preview=False
                )
                ...
                result = builder.article(
                    "@majesteuserbot",
                    text="""@majesteuserbot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("❌ Hey! Benim mesajlarımı düzenlemeye kalkma! Kendine bir @majesteuserbot kur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"** En Gelişmiş UserBot!** [Majeste](https://t.me/majesteuserbot) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
